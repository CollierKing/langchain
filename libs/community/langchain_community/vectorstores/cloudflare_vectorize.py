...
class CloudflareVectorize(VectorStore):
    ...
    def set_api_token(cls, api_token: str) -> None:
        """Set the API token at the class level for all instances and class methods."""
        globals()["_api_token"] = api_token
    
    @classmethod
    def get_api_token(cls) -> str:
        """Get the API token from class level or raise an error if not set."""
        if globals().get("_api_token") is None:
            raise ValueError("API token not set. Call CloudflareVectorize.set_api_token() first.")
        return globals().get("_api_token")
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.d1_base_url = kwargs.get("d1_base_url")
        
        # Use the provided API token or get from class level
        self.api_token = api_token or self.get_api_token()
        ...
        self.d1_headers = {
            "Authorization": f"Bearer {self.d1_api_token}",
            "Content-Type": "application/json",
        }
        self.d1_database_id = kwargs.get("d1_database_id")
        self.ai_api_token = kwargs.get("ai_api_token")
        self.vectorize_api_token = kwargs.get("vectorize_api_token")
    ...
    def _get_url(self, endpoint: str) -> str:
        ...
        return f"{self.base_url}/accounts/{self.account_id}/vectorize/v2/indexes/{self.index_name}/{endpoint}"
    ...
    # MARK: - ad1_create_database
    async def ad1_create_database(self, database_name: str) -> Dict[str, Any]:
        """Asynchronously create a D1 database.
        
        Args:
            database_name: Name for the new database
            
        Returns:
            Response data with database information
        """
        import httpx
        
        async with httpx.AsyncClient() as client:
            response = await client.post(
                self._get_d1_url("database"),
                headers=self._get_d1_headers(),
    ...
    async def ad1_create_table(self, database_id: str, table_schema: str) -> Dict[str, Any]:
        """Asynchronously create a table in a D1 database using SQL schema.
        
        Args:
            database_id: ID of the database to create table in
            table_schema: SQL schema for table creation (CREATE TABLE statement)
    ...
    # MARK: - ad1_get_texts
    async def ad1_get_texts(self, database_id: str, table_name: str, filter_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def similarity_search(
        ...
        **kwargs: Any,
        ...
        docs_and_scores = self.similarity_search_with_score(
            query=query, k=k, filter=filter, namespace=namespace, **kwargs
        )
        # Return just the documents (first element of the tuple)
    ...
    def delete(
        ...
        **kwargs: Any
        ...
        response = requests.post(
            self._get_url("delete_by_ids"),
            headers=self._headers,
            json=delete_request,
        )
        response.raise_for_status()
        
        self.d1_delete_texts(
            database_id=self.d1_database_id,
            table_name=self.table_name,
            ids=ids
        )
    ...
    async def adelete(
        ...
        **kwargs: Any
        ...
        if namespace:
            delete_request["namespace"] = namespace
    ...
    def get_by_ids(
        ...
        response = requests.post(
            self._get_url("get_by_ids"),
            headers=self._headers,
            json=get_request,
        )
        response.raise_for_status()
        
        vector_data = response.json().get("result", {}).get("vectors", [])
        
        # Get text data from D1 database
        d1_response = self.d1_get_texts(
            database_id=self.d1_database_id,
            table_name=self.table_name,
            filter_params={"id": {"$in": ids}}
        )
        
        # Create a lookup dictionary for D1 text data by ID
        d1_data_by_id = {}
        if d1_response and "results" in d1_response:
            for item in d1_response["results"]:
                if "id" in item:
    ...
    async def aget_by_ids(
        ...
        return response_data.get("result", {}).get("vectors", [])
    ...
    async def aget_index_info(self) -> Dict[str, Any]:
    ...
    def create_metadata_index(self, property_name: str, index_type: str = "string") -> Dict[str, Any]:
        ...
        """
        response = requests.post(
            f"{self._get_url('metadata_index/create')}",
            headers=self._headers,
            json={
    ...
    async def acreate_metadata_index(self, property_name: str, index_type: str = "string") -> Dict[str, Any]:
    ...
    def list_metadata_indexes(self) -> List[Dict[str, str]]:
        ...
        """
        response = requests.get(
            f"{self._get_url('metadata_index/list')}",
            headers=self._headers,
        )
        response.raise_for_status()
    ...
    async def alist_metadata_indexes(self) -> List[Dict[str, str]]:
    ...
    def delete_metadata_index(self, property_name: str) -> Dict[str, Any]:
        ...
        """
        response = requests.post(
            f"{self._get_url('metadata_index/delete')}",
            headers=self._headers,
    ...
    async def adelete_metadata_index(self, property_name: str) -> Dict[str, Any]:
    ...
    def get_index(self) -> Dict[str, Any]:
        ...
        """
        response = requests.get(
            f"{self.base_url}/accounts/{self.account_id}/vectorize/v2/indexes/{self.index_name}",
            headers=self._headers,
        )
        response.raise_for_status()
    ...
    # MARK: - aget_index
    async def aget_index(self) -> Dict[str, Any]:
    ...
    def delete_index(
        ...
        response = requests.delete(
            f"{base_url}/accounts/{account_id}/vectorize/v2/indexes/{index_name}",
            headers=headers,
        )
        response.raise_for_status()
